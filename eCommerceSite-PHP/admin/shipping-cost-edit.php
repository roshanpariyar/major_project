...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['state_id'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM tbl_shipping_cost WHERE state_id=? and state_id!=?");
    	$statement->execute(array($_POST['state_id'],$current_country));
    	$total = $statement->rowCount();							
    	if($total) {
    		$valid = 0;
        	$error_message .= 'state already exists<br>';
    	}
    }

    if($valid == 1) {    	
		// updating into the database
		$statement = $pdo->prepare("UPDATE tbl_shipping_cost SET state_id=?,amount=? WHERE shipping_cost_id=?");
		$statement->execute(array($_POST['state_id'],$_POST['amount'],$_REQUEST['id']));
