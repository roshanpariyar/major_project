...
if(!defined("LANG_VALUE_98")){
define("LANG_VALUE_98",'Retype Password');	
 

    }if(!defined("LANG_VALUE_99")){
        define("LANG_VALUE_99",'Update Password');
        }
        if(!defined("LANG_VALUE_100")){
        define("LANG_VALUE_100",'New Password');}	
        if(!defined("LANG_VALUE_101")){
        define("LANG_VALUE_101",'Retype New Password');}	
            
            if(!defined("LANG_VALUE_138")){
     define("LANG_VALUE_138",' Password can not be empty.');	
    }	
...
if (isset($_POST['form1'])) {
    ...
    if( empty($_POST['cust_password']) || empty($_POST['cust_re_password']) ) {
        $valid = 0;
        $error_message .= LANG_VALUE_138."<br>";/*138*/
    }

    if( !empty($_POST['cust_password']) && !empty($_POST['cust_re_password']) ) {
        if($_POST['cust_password'] != $_POST['cust_re_password']) {
            $valid = 0;
            $error_message .= LANG_VALUE_139."<br>";/*139*/
        }
    }
    ...
    if($valid == 1) {
        ...
        $success_message = 'Password is updated successfully';/*141*/
    }
}
?>
